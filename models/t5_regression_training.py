...
def evaluate_model(model, dataloader, qt, fold, epoch=None, step=None):
    model.eval()
...
for fold in folds:
    ...
    for epoch in range(num_epochs):
        ...
        for batch in tqdm(train_dataloader):
            ...
            if step % 2000 == 0 and step > 0 and step < num_training_steps-5:
                torch.save(model.state_dict(), f"{MODEL_NAME}_survival_epoch_{epoch}_step_{step}_{fold}.pth")
    ...
    torch.save(model.state_dict(), f"{MODEL_NAME}_survival_{fold}.pth")
