...
def predict_with_cox_save(model_path: str, features_path: str, data_csv: str, output_json: str):
    """
    Load a saved Cox model and feature names, predict on a CSV dataset, and save results as JSON.
    """

    # Columns to keep and categorical variables
    cols_to_keep = [
        "gesl", "tumsoort", "topo_sublok", "later", "morf", "gedrag", "diffgrad",
        "er_stat", "pr_stat", "her2_stat", "leeft", "incjr", "time_to_os", "os"
    ]
    ...
    # Load CSV
    df = pd.read_csv(data_csv, sep=";")  # or sep="," depending on your CSV
    ...
    for col in feature_names:
        if col not in df:
            df[col] = 0
    df = df[feature_names]  # order columns exactly as in training
    df = df.fillna(0)
...
if __name__ == '__main__':
    ...
    predict_with_cox_save(
        ...
        features_path="cox/cox_full_features.joblib",
        data_csv="csv_data/real_data.csv",
        output_json="cox/cox_real_data_result.json"
    )
